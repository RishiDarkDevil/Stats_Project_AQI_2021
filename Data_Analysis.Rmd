...
```{r Model_Yearly_AQI, fig.asp=1.6, fig.width=12, warning=FALSE, message=FALSE, results='hide'}
...
```{r Model_AQI_First6Month, fig.asp=2.4, fig.width=12, warning=FALSE, message=FALSE, results='hide'}
# First 6 Months
...
```{r Model_AQI_Last6Month, fig.asp=1.6, fig.width=12, warning=FALSE, message=FALSE, results='hide'}
# AQI - Last 6 Month
air_data_pollutants_2019_avg_median <- air_data_india_Pollutants_nonpollutants_daily %>%
  filter(Year == 2019 & Measure == "Avg_Max" & !(Month %in% c(1,2,3,4,5,6))) %>% #  -----------------------------------------------LOOOOK!!!!!!!!!!
  select(-c("non_pollutants", "Levels"))
...
```{r Model_AQI_Indiv_First6Month, fig.asp=4.8, fig.width=12, warning=FALSE, message=FALSE, results='hide'}
...
```{r Model_AQI_Indiv_Last6Month, fig.asp=3.2, fig.width=12, warning=FALSE, message=FALSE, results='hide'}
...
In this section, we will see how interestingly one pollutant's AQI affect the other pollutant's AQI, as well as we will study the natural effect of the non-pollutants on the pollutant's AQI values. We have already seen in the relationships part of Visual Overview, that there indeed exist a strong relationship between the pollutants and we also saw before that that the AQI values are quite dependent on the Month! Now, it is natural to assume that the contribution of Factories and Transportation and other factors are uniformly distributed over all months, then where does this pattern turn up ? It turns out that it can be because of the change in season and along with it the temperature, humidity, pressure, etc. also change which might be the actual cause in the difference in the concentration of the Pollutants measured. Thus, it can very well be that the relationship of Pollutants with Month is a Spurious one and the actual roots lie in the measure of non-pollutants which undergo change with time.

We will also explore interesting ideas like how much one of the pollutants contribute to the AQI Levels of the other pollutants?

Finally we will as is with the all the other section try out some interesting ideas to see whether the First 6 Months of 2020 actually had low AQI Levels of pollutants.

### REGRESSION

The main model which will be focusing on is the Multiple Linear Regression.

-   As we were doing earlier we will have a separate Analysis for the first 6 months of the Year 2019, 2020 & 2021.
