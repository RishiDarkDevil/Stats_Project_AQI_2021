...
# Predicting top 9 cities pollutants from non pol trained data
predict_all_cities_pol <- function(data, heading = "Residuals"){
  # Non-pollutants based predictions --- MAKE SURE THE model_s1 and all model used in this part is trained with non-pollutants
  ...
  return(annotate_figure(ggarrange(res_so2, res_no2, res_co, res_o3, res_pm10, res_pm25), top = text_grob(heading, face = "bold", size = 18)))
}
```
...
```{r Model_2019_resid, warning=FALSE, message=FALSE, fig.asp=0.618, fig.width=14}
air_data_india_All_Specie_Daily_Regress_model <- air_data_india_All_Specie_Daily_Regress %>%
...
suppressMessages(resplot <- predict_all_cities_pol(air_data_india_All_Specie_Daily_Regress_model, "Residuals of MLR Model of Pollutant Levels on Weather Parameters(First 6 Months)"))
resplot
...
```{r Model_2019_resid_and_predict_resid, warning=FALSE, message=FALSE, fig.asp=1.4, fig.width=12}
air_data_india_All_Specie_Daily_Regress_model <- air_data_india_All_Specie_Daily_Regress %>%
  filter(Year == 2019) %>% # ------------LOOOOOOOK
  filter(Month %in% c(1,2,3,4,5,6))
...
suppressWarnings(suppressMessages(hist_compare <- comparing_year_residuals(air_data_india_All_Specie_Daily_Regress, "2019", c(1,2,3,4,5,6))))
...
t_reg0 <- ggtexttable(prediction.R2$Prediction_Table[1:3]%>%
                        mutate(across(where(is.numeric), ~ round(., digits = 2))), theme = ttheme("mBlue"), rows = NULL) %>%
  tab_add_title("Model R2 on 2019 Data", face = "bold")
...
t_reg1 <- ggtexttable(prediction.R2$Prediction_Table[1:3]%>%
                        mutate(across(where(is.numeric), ~ round(., digits = 2))), theme = ttheme("mViolet"), rows = NULL) %>%
  tab_add_title("Model R2 on 2020 Data", face = "bold")
...
t_reg2 <- ggtexttable(prediction.R2$Prediction_Table[1:3]%>%
                        mutate(across(where(is.numeric), ~ round(., digits = 2))), theme = ttheme("mCyan"), rows = NULL) %>%
  tab_add_title("Model R2 on 2021 Data", face = "bold")

model_performance <- ggarrange(t_reg0, t_reg1, t_reg2, ncol = 3)

ggarrange(hist_compare, model_performance, nrow = 2, heights = c(10, 2))
```
...
```{r Model_2020, warning=FALSE, message=FALSE, fig.asp=1.3, fig.width=16.2}
air_data_india_All_Specie_Daily_Regress_model <- air_data_india_All_Specie_Daily_Regress %>%
  filter(Year == 2020) %>% # ------------LOOOOOOOK
  filter(Month %in% c(1,2,3,4,5,6)) #%>% # ------------LOOK
