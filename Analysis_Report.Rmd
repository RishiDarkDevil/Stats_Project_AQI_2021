...
-   Here we will look at *how the different pollutants are distributed and how different are the models for different year & location.*
...
air_data_india_Pollutants_nonpollutants_daily %>%
  mutate(Year = as.character(Year)) %>%
  ggplot(aes(AQI, ..density.., fill = Year)) +
  geom_histogram(binwidth = 0.5) +
  facet_wrap(~City + Specie, ncol = 6) +
  labs(fill = "City", x = "Pollutant Levels", y = "Density")+
  theme(legend.position = "bottom") +
  guides(
    fill = guide_legend(
      nrow = 1,
      aes.override = list(size = 3)
    )
  ) +
  scale_fill_viridis(discrete = TRUE)
```
